 ...
 *
 * A sheet is 8.5 inches wide and does not negotiate (§4), so the only honest way
 * to show one beside an option panel is to scale it down. `transform: scale()`
 * and not a smaller sheet: every length on the paper stays exactly what it says
 * it is, the type is still real selectable text, and what is on screen is the
 * page that will come out of the printer rather than a drawing of it.
 *
 * The scale is measured rather than guessed — Letter portrait beside a panel on
 * a laptop is about 0.45 and A4 landscape on a phone about 0.25, and a
 * hard-coded number would be wrong on both.
 *
 * Print undoes all of it: `print.css` hides everything that isn't paper and
...
export function Preview({ sheets }: { sheets: Sheet[] }) {
  ...
  // The room there is, and the stack's own unscaled size. ResizeObserver
  // reports layout boxes, so the stack's box is what it would measure at 1:1,
  // uncontaminated by the transform applied to it.
 ...
 *
 * Two renders rather than one un-scaled at print time: `transform` on an
 * ancestor of a page box is one of the reliable ways to lose the bottom of a
 * sheet, and print is the entire output path here (§10) — there is no PDF to
 * notice it in first. So the preview is screen-only and this is print-only, and
 * neither has to be undone under pressure. It costs a second copy of the markup
 * in the DOM and nothing on paper.
 */
