...
export function GrammarPanel({ config, set }: PanelProps<GrammarConfig>) {
  ...
  return (
    <>
      ...
      {/* Columns only where a sentence and a slot share a line. Both the offer
          and the cap are `grammarColumns` rather than a topic named here: the
          engine already decides this, and a second copy of the rule is a
          stepper that moves a number the paper ignores. */}
      <Sizing
        label="Sentences"
        count={config.count}
