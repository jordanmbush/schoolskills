...
/**
 * The problems block of each page, in order. A time sheet prints one such
...
describe("how much fits", () => {
  ...
  it("draws nothing when not even one row fits", () => {
    ...
    const tall = config({ style: "elapsed", fontPt: 200 });
    expect(timeLayout(tall).perPage).toBe(0);
