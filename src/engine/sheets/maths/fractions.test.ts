...
describe("the fractions family", () => {
  ...
  it("gives the sheet a title and a score box it can be marked against", () => {
    ...
    // Out of what is printed, not out of what was asked for.
...
describe("how much fits", () => {
      ...
      for (const margin of MARGINS) {
        for (const fontPt of [12, 18]) {
          for (const shape of EVERY_SHAPE) {
            ...
            const { row, perPage } = fractionLayout(config(over));
  ...
  it("honors the count and the columns it was given", () => {
    ...
    const drawn = pagesOf(
      buildSheet(config({ style: "identify", columns: 6 }), 1),
    )[0];
    expect(drawn.columns).toBe(3);
  });

  it("runs on to another page rather than cutting the count to the paper", () => {
    // Fifty problems are as many pages as fifty take, not one page's worth:
    ...
    const { perPage } = fractionLayout(config(over));
    const pages = pagesOf(sheet);
  ...
  it("draws nothing when not even one row fits", () => {
    ...
    expect(fractionLayout(config(over)).perPage).toBe(0);
