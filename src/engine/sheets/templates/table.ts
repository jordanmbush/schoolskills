 ...
 * same two questions before they know what fits on the page. So the arithmetic
 * is written once here rather than twice in the families, for the reason
 * `chrome.ts` gives about the header: a second copy would be a second thing to
 * keep in step, and the failure is silent on screen and is the last row of the
 * table on a second sheet of paper.
 *
 * **The columns sum to the box exactly.** A table is drawn from a list of
 * widths, so a table whose widths came to a few thousandths less than the page
 * would print with a ragged right edge that no test would notice and every
 * reader would. Dividing by shares leaves a remainder of up to one mil per
 * column; it goes to the widest column, which is the one place it cannot be
 * seen.
 */
 ...
 *
 * Taller than the type it holds because a heading is read across a table rather
 * than down a line, and because it is the one row a child does not write in —
 * making it exactly as tall as a writing row would leave a reader hunting for
 ...
 * for one — the verse chart is a verse and then a week, and a verse sized to
 * the whole page would leave the week a row shorter than the writing in it,
 * which overflows downwards and off the paper.
 */
 ...
 *
 * Shares rather than lengths because the families that declare them are
 * declaring proportions: a reading log's "Minutes" column is as wide as three
 * numerals and its "Book" column is as wide as a title, and neither is a number
 * of inches anybody should have to write down twice — once for Letter and once
 * for A4.
 */
export function tableColumns(shares: ColumnShare[], width: Mil): TableColumn[] {
  ...
  // the margin — which is a line a reader sees against the edge of the paper.
  //
 ...
 *
 * The count is a request and what fits is the answer, exactly as it is for
 * problems (§4). Between the two bounds the rows stretch to fill the page,
 * which is what makes the same block a *log* at eight rows and a *chart* at
 * twenty: a chore chart four rows down a page with nothing under it is a chart
 * nobody would pin up.
 *
