 ...
 *   - **A split is exhaustive or it is absent.** Every word has to go somewhere,
 *     so `dog` alone leaves `The big brown` homeless. Only plain statements get
 *     one — a command's subject is not on the page, and a question's is inside
 *     the verb phrase.
 ...
 *     a sheet is printed on both sides of the Atlantic.
 */

/**
 * The five word classes a sheet may ask about.
 *
 * Five rather than the eight or nine a grammar names. Determiners, prepositions
 * and conjunctions are all taught by a primary scheme and all three are classes
 * the schemes disagree about — `the` is a determiner in one classroom and an
 * adjective in the next, and a sheet cannot mark a child on which classroom
 * they are in.
 */
 ...
 *
 * A command and a statement both end in a full stop, and telling those two
 * apart is the whole of the exercise. The other two carry their mark, which is
 * the easy half and is what makes the sheet answerable by a six-year-old.
 */
...
/**
 * The whole sentence, cut once: the *complete* subject and predicate, so every
 * word is in one or the other. That exhaustiveness is what makes the question
 * markable at all — see the house rules above.
 */
...
export type Tagged = {
   ...
   *
   * It is also the tag that sizes the smallest topic in the bank, so a sentence
   * that can honestly carry a name, a place or a day should carry one.
   * `grammar.test.ts` holds every topic above a full page of itself for exactly
   * this reason: a supply of thirteen against a page that holds seventeen is
   * the same sheet whatever seed a parent rerolls to.
   */
  proper?: string;
};
...
/* ── Statements ────────────────────────────────────────────────────────────
   ...
   twelve sentences are the whole supply is the same page every week.        */
...
/* ── Questions ─────────────────────────────────────────────────────────────
   Every one inverted or opened by a question word, so what makes it a question
   is on the page rather than in the reader's voice: "You are coming" is a
   question or a statement depending on how it is said.                      */
...
/* ── Commands ──────────────────────────────────────────────────────────────
   The kind that shares its mark with a statement, which is what makes a
   sentence-types sheet an exercise rather than a look at the last character.
   Every one opens on a verb that is not also a noun: `Put`, not `Pass`;
   `Choose`, not `Close`.                                                    */
 ...
 *
 * Grouped above by kind because the constraints on a command are not the
 * constraints on a statement, and flattened here because a topic is a filter
 * over the whole bank — one that had to know which array to look in could look
 * in the wrong one.
 */
 ...
 *
 * Done by lower-casing the first letter of the word rather than the word
 * itself, so this and `text` can only ever differ by that one character — which
 * is what makes "exactly one word to put right" a property a test can check
