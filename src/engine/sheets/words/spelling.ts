/**
 * Spelling, from whatever list a parent already has. Seven styles are one
 * family rather than seven because none of them changes what the sheet is
 * about — the words do, and the style is a control beside them (§11).
 *
 * Being handed its content rather than generating it is also what makes it the
 * family the three bootstraps land on — two of them are "here is a list", and
 * the third is the same thing with the list chosen for you (§14).
 */
 ...
 * under its question — 1.35 for each line of type and the gap between them,
 * rounded *up*, because a row under-reserved is the last question of the page
 ...
 *
 * Takes any list-carrying config rather than a `WordsConfig`, so that the
 * puzzle family shares it: a second sanitiser would be a second answer to "is
 * `Cat` the same word as `cat`", and the two sheets would disagree about a list
 ...
 *
 * Never the first one: the point of the exercise is the letters inside a word
 * a child can already see the shape of.
 *
 ...
 * answer key that prints only the first of them.
 *
 * A word with no interior letters to take — a single letter, an initial — is
 * printed whole rather than blanked to nothing. A short word that cannot spare
 * as many as were asked for gives up the ones it can.
 */
 ...
 *
 * One column whatever the config says: the `blanks` and `choice` blocks are
 * both flex columns of full-width items, and a family that asked either of them
 * for two columns would be declaring a layout the renderer does not have.
 */
...
function rowHeight(config: WordsConfig): Mil {
  ...
  switch (config.style) {
    ...
    default:
      ...
      // lines has to be reserved for here. That gap is `WRAP_GAP`.
      return (
...
/** How many words the paper holds, and how wide a column of them is (§4). */
...
function problemOf(word: string, config: WordsConfig): Problem {
  if (config.style === "test") {
    // Nothing to read: the word is said aloud and written on the rule. The key
    // is the list, which is what makes the sheet markable by somebody who was
    // not the one reading it out.
    return { prompt: "", answer: word };
  }
  const times = timesOf(config);
  if (config.style === "sentence") {
    // Empty on the key as well: what a child puts there is their own sentence,
    // so there is no answer to print. The word still sits above the lines — it
    // is the question — and the lines are the answer place, which is why there
    // is no ruled slot as well (see `Problem.answers`).
    return {
      prompt: word,
      answer: "",
 ...
 * with the word printed next to it.
 *
 * The bank is shuffled, and it is the one style where that is right: every list
 * this sheet is likely to be set on arrives alphabetical already — the shipped
 * Dolch lists are, and so is anything typed out of a dictionary — and a sheet
 * whose answer column is its own question column is not a sheet. Which scramble
 * is the seed's, so "another one like this" is another order (§7).
 */
...
/**
 * One word among the words it is most easily mistaken for — the race's own
 * `wordDistractors`, so a printed sheet and a played round ask one question of
 * one list (§11).
 */
const findQuestion = (
  word: string,
...
export const WORDS_SHEET: SheetSpec<WordsConfig> = {
  ...
  // and what goes back in — so a key cannot disagree with its sheet.
  key: (sheet) => ({
    ...sheet,
    answers: true,
