...
### 4.7 · The board is life-size

Key pitch on every full-size keyboard a child will meet is **19.05mm** — three
quarters of an inch, which CSS defines as exactly 72px, which is `4.5rem`. That
is the ceiling of `--key`, and it is a measurement rather than a taste.

The reason is what the board is _for_. It is not an illustration of a keyboard;
it is a map a child glances at while their hands are on the real one. A map at
half scale asks them to do the scaling — to see that the distance between `f`
and `j` on screen is the distance their two index fingers are already holding —
and that translation is exactly the work that looking down at their hands would
have saved them. At pitch, the picture and the plastic are the same shape, so
the glance costs nothing and the reach it teaches is the reach they make.

Written in `rem` rather than `mm`, which are identical lengths (CSS pins an inch
to 96px), so that a child who has turned their text up gets a board that grew
with everything else.

The ceiling was `2.4rem` — about half pitch — through #133–#156, which on any
laptop drew a keyboard the size of a phone in the middle of a half-empty screen.
Three things had to give for the real number to fit:

- **The `vw` term**, 5.6 → 6, so the board is 90% of a narrow viewport rather
  than 84%. On a phone and a portrait tablet the width still decides, and the
...
- **The race column.** `.race` is `min(720px, 100%)` because that is a
  ...
  overflowing grid item. Half the column minus half the board means the same
  thing in both directions.
- **The height term, on the passage screen only.** `9dvh` is a fraction of the
  viewport and what the board competes with there is not one: the HUD, the pass
  bars or the ghost lane, the line you type on, four grid gaps and the race's
  own padding come to about 365px at every height, and two lines of passage left
  to read is another 60. A coefficient that clears 425px of chrome at an 850px
  viewport does not clear it at 700, and `.race` is `min-height` — so the board
  grew the page instead of itself, and a race that scrolls under a thumb
  mid-word is the one thing that `min-height` is there to prevent. At half pitch
  the ceiling hid this; the dvh term was never the governor it looked like. So
  `.race.typing` re-declares `--key` with the constant subtracted and the
  remainder divided by the board's own height in key units, `5 + 4 × 0.09`:

  ```css
  min(6vw, 9dvh, calc((100dvh - var(--ad-h) - 26.5rem) / 5.36))
  ```

  This is the second declaration of `--key` that decision 37 rules out, and it
  is not the one it rules out. What may never happen is a falling letter's lane
  and the cap it names coming from two numbers — and the passage screen has no
