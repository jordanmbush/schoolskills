...
Five more, for worksheets. A `@font-face` is lazy, so a page that never draws a
...
| `playwrite-us-modern-400.woff2` | Playwrite US Modern | TypeTogether (Veronika Burian, José Scaglione) | https://fonts.google.com/specimen/Playwrite+US+Modern |
| `playwrite-gb-j-400.woff2`      | Playwrite GB J      | TypeTogether (Veronika Burian, José Scaglione) | https://fonts.google.com/specimen/Playwrite+GB+J      |
| `opendyslexic-latin-400.woff2`  | OpenDyslexic        | Abbie Gonzalez                                 | https://github.com/antijingoist/opendyslexic          |

Andika is the print face and OpenDyslexic the accessibility option; the three
Playwrite files are the three cursive **models** a sheet can be set in — the
looped American hand, the unlooped one, and the fully joined British one.
`src/styles/fonts.css` says why each was chosen and
...
- Andika and the three Playwrites are what `fonts.googleapis.com/css2` resolves
  to for those families — Andika's `latin` block, and Playwrite's single block
...
c399b84741144cb901467db9e578e3c13aa0971dfd91977e9e9522c5f12e416d  playwrite-us-modern-400.woff2
d5b6f3008939ed7bbefd4155ede56be28a9a4e372459bd497df63b184d5f88f6  playwrite-gb-j-400.woff2
...
recorded here. `src/engine/sheets/hands/print.ts` and `hands/cursive.ts` are
sets of letters stored as pen strokes — single lines down the middle of each
stroke, for tracing — and those lines were traced over the outlines in the
faces' published sources:

- the print hand over Andika's (`Andika-Regular.ufo` at
  https://github.com/silnrsi/font-andika, OFL 1.1);
- the cursive hand over Playwrite's (`Playwrite_MM.glyphspackage` at
  https://github.com/TypeTogether/Playwrite, OFL 1.1), at the point between its
  masters that is the US Trad model, with the alternates that model chooses —
  the `tracedOver` table in `art/hands/cursive/hand.json` names each one.
...
own header, and they are called "School Skills Print" and "School Skills
Cursive", which are nobody's Reserved Font Names. The drawings themselves are
in `art/hands/`; `docs/printables.md` §25 says how one becomes the other.
...
- `Copyright 2023 The Playwrite Project Authors (https://github.com/TypeTogether/Playwrite)`
  (all three models — the superfamily ships one notice)
...
The other eight carry only `name` ID 14, a URL pointing at the OFL — which is
